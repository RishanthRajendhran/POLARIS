...
def _scalarize_metric_value(val):
    ...
    if isinstance(val, np.ndarray):
        if val.size == 0:
            return None
        return float(np.asarray(val, dtype=np.float32).mean())
    if isinstance(val, (list, tuple)):
        if len(val) == 0:
            return None
        scalars = []
        for item in val:
            item = _scalarize_metric_value(item)
            if item is not None:
                scalars.append(item)
        if not scalars:
            return None
...
def _scalarize_metric_dict(metrics):
    out = {}
    for key, val in metrics.items():
        scalar = _scalarize_metric_value(val)
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        if "sample_weights_gt" in data.batch.keys():
            select_keys.append("sample_weights_gt")
        if "is_gt" in data.batch.keys():
            select_keys.append("is_gt")
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    sample_weights_gt = model_inputs.get("sample_weights_gt", None)
                    eff_advantages = advantages
                    if sample_weights_gt is not None:
                        eff_advantages = eff_advantages * sample_weights_gt.unsqueeze(-1)

                    micro_batch_metrics["actor_debug/nonfinite_advantages_count"] = float((~torch.isfinite(advantages)).sum().item())
                    micro_batch_metrics["actor_debug/nonfinite_eff_advantages_count"] = float((~torch.isfinite(eff_advantages)).sum().item())
                    micro_batch_metrics["actor_debug/nonfinite_old_log_prob_count"] = float((~torch.isfinite(old_log_prob)).sum().item())
                    micro_batch_metrics["actor_debug/nonfinite_log_prob_count"] = float((~torch.isfinite(log_prob)).sum().item())
                    if rollout_is_weights is not None:
                        micro_batch_metrics["actor_debug/nonfinite_rollout_is_weights_count"] = float((~torch.isfinite(rollout_is_weights)).sum().item())

                    # Compute policy loss (any function is expected to return 2 values)
                    pg_loss, pg_metrics = policy_loss_fn(
                        old_log_prob=old_log_prob,
                        log_prob=log_prob,
                        advantages=eff_advantages,
                        response_mask=response_mask,
                        loss_agg_mode=loss_agg_mode,
                        config=self.config,
                        rollout_is_weights=rollout_is_weights,
                    )
                    micro_batch_metrics.update(_scalarize_metric_dict(pg_metrics))
                    micro_batch_metrics["actor_debug/nonfinite_pg_loss"] = float(not torch.isfinite(pg_loss).item())

                    if "is_gt" in model_inputs:
                        is_gt_rows = model_inputs["is_gt"].bool()
                        for split_name, split_mask in (("gt", is_gt_rows), ("policy", ~is_gt_rows)):
                            if not bool(split_mask.any()):
                                continue
                            split_old_log_prob = old_log_prob[split_mask].detach()
                            split_log_prob = log_prob[split_mask].detach()
                            split_advantages = eff_advantages[split_mask].detach()
                            split_response_mask = response_mask[split_mask]
                            split_rollout_is_weights = (
                                rollout_is_weights[split_mask].detach() if rollout_is_weights is not None else None
                            )
                            split_pg_loss, _ = policy_loss_fn(
                                old_log_prob=split_old_log_prob,
                                log_prob=split_log_prob,
                                advantages=split_advantages,
                                response_mask=split_response_mask,
                                loss_agg_mode=loss_agg_mode,
                                config=self.config,
                                rollout_is_weights=split_rollout_is_weights,
                            )
                            micro_batch_metrics[f"actor_split/{split_name}_pg_loss"] = split_pg_loss.detach().item()
                            micro_batch_metrics[f"actor_split/{split_name}_nonfinite_pg_loss"] = float(
                                not torch.isfinite(split_pg_loss).item()
                            )
                            split_eff_adv = torch.masked_select(split_advantages, split_response_mask.bool())
                            if split_eff_adv.numel() > 0:
                                micro_batch_metrics[f"actor_split/{split_name}_eff_adv_abs_mean"] = (
                                    split_eff_adv.abs().mean().detach().item()
                                )
                            micro_batch_metrics[f"actor_split/{split_name}_rows"] = float(int(split_mask.sum().item()))
                        ...
                        if "is_gt" in model_inputs:
                            ...
                            for split_name, split_mask in (("gt", is_gt_rows), ("policy", ~is_gt_rows)):
                                if bool(split_mask.any()):
                                    split_entropy = agg_loss(
                                        loss_mat=entropy[split_mask],
                                        loss_mask=response_mask[split_mask],
                                        loss_agg_mode=loss_agg_mode,
                                    )
                                    micro_batch_metrics[f"actor_split/{split_name}_entropy"] = split_entropy.detach().item()
                        micro_batch_metrics["actor_debug/nonfinite_entropy_count"] = float((~torch.isfinite(entropy)).sum().item())
                        if entropy_coeff != 0:
                    ...
                    if self.config.use_kl_loss:
                        ...
                        micro_batch_metrics["actor_debug/nonfinite_ref_log_prob_count"] = float((~torch.isfinite(ref_log_prob)).sum().item())
                        micro_batch_metrics["actor_debug/nonfinite_kl_loss"] = float(not torch.isfinite(kl_loss).item())
                        if "is_gt" in model_inputs:
                            is_gt_rows = model_inputs["is_gt"].bool()
                            for split_name, split_mask in (("gt", is_gt_rows), ("policy", ~is_gt_rows)):
                                if bool(split_mask.any()):
                                    split_kld = kl_penalty(
                                        logprob=log_prob[split_mask].detach(),
                                        ref_logprob=ref_log_prob[split_mask].detach(),
                                        kl_penalty=self.config.kl_loss_type,
                                    )
                                    split_kl_loss = agg_loss(
                                        loss_mat=split_kld,
                                        loss_mask=response_mask[split_mask],
                                        loss_agg_mode=loss_agg_mode,
                                    )
                                    micro_batch_metrics[f"actor_split/{split_name}_kl_loss"] = split_kl_loss.detach().item()
                    ...
                    append_to_dict(metrics, _scalarize_metric_dict(micro_batch_metrics))

                grad_norm = self._optimizer_step()
                mini_batch_metrics = {"actor/grad_norm": grad_norm.detach().item()}
                append_to_dict(metrics, _scalarize_metric_dict(mini_batch_metrics))
        self.actor_optimizer.zero_grad()
        return metrics
