...
def _to_list_B(val, B):
    """Ensure val is a list of length B for robust non_tensor_batch chunking.
    Handles scalars, dicts, strings, None, 0-D arrays by broadcasting to [val]*B."""
    try:
        if isinstance(val, (list, tuple)) and len(val) == B:
            return list(val)
    except Exception:
        pass
    try:
        arr = np.asarray(val, dtype=object)
        if getattr(arr, "ndim", 0) >= 1 and arr.shape[0] == B:
            return list(arr)
    except Exception:
        pass
    return [val] * B
...
class DataProto:
    ...
    def chunk(self, chunks: int) -> list["DataProto"]:
        ...
        # Compute sizes for each chunk (handles uneven splits from padding/balancing)
        if bsz_in_batch is not None:
            sizes = bsz_in_batch.tolist()
        else:
            B = len(self)
            sizes = [B // chunks] * chunks

        B_total = sum(sizes)

        # Split non-tensor batch robustly (broadcast 0-D values, then slice by sizes)
        non_tensor_batch_lst = [{} for _ in range(chunks)]
        for key, val in self.non_tensor_batch.items():
            base_list = _to_list_B(val, B_total)
            start = 0
            for i, s in enumerate(sizes):
                piece = base_list[start:start + s]
                non_tensor_batch_lst[i][key] = np.array(piece, dtype=object)
                start += s
