...
class RewardLoopWorker:
    ...
    async def compute_score_batch(self, data: DataProto) -> list[dict]:
        # If the reward manager provides its own batch path, use it (e.g. composite_batch)
        if hasattr(self.reward_manager, "compute_score_batch"):
            return await self.reward_manager.compute_score_batch(data)
        tasks = []
