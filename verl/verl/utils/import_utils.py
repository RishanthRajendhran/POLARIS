...
def load_module(module_path: str, module_name: Optional[str] = None) -> object:
    ...
    if module_path.startswith(PKG_PATH_PREFIX):
        ...
        # If loading an __init__.py, set up as a proper package so relative
        # imports (e.g. ``from .engine import ...``) work correctly.
        abs_module_path = os.path.abspath(module_path)
        is_package = os.path.basename(abs_module_path) == "__init__.py"
        if is_package:
            pkg_dir = os.path.dirname(abs_module_path)
            spec = importlib.util.spec_from_file_location(
                spec_name, abs_module_path,
        ...
        if is_package:
            ...
            module.__package__ = spec_name
            sys.modules[spec_name] = module
            # Walk up to find the top-level package root: the highest ancestor
            # directory that contains an __init__.py.  Add its parent to
            # sys.path so that absolute imports like ``from reward.components.base``
            # resolve when the package lives outside the normal Python path.
            pkg_root = pkg_dir
            parent = os.path.dirname(pkg_root)
            while parent and parent != pkg_root and os.path.isfile(os.path.join(parent, "__init__.py")):
                pkg_root = parent
                parent = os.path.dirname(pkg_root)
            top_level_parent = os.path.dirname(pkg_root)
            if top_level_parent and top_level_parent not in sys.path:
                sys.path.insert(0, top_level_parent)
