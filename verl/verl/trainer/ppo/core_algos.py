...
def compute_grpo_outcome_advantage(
    ...
    is_gt: Optional[np.ndarray] = None,
    ...
    Returns:
        ...
        Returns: `(torch.Tensor)`
            shape is (bs, response_length)
    """
    # Outcome score reduction mode:
    # - "sum"      : original VERL behavior (scales with response length)
    # - "sum_norm" : divide by fixed response_length
    # - "mean"     : divide by actual nonpad response length
    score_mode = "sum"
    if config is not None:
        try:
            score_mode = str(config.get("grpo_score_mode", "sum"))
        except Exception:
            score_mode = "sum"

    if score_mode == "sum_norm":
        scores = token_level_rewards.sum(dim=-1) / max(1, token_level_rewards.shape[-1])
    elif score_mode == "mean":
        denom = response_mask.sum(dim=-1).clamp_min(1.0)
        scores = (token_level_rewards * response_mask).sum(dim=-1) / denom
    else:
        scores = token_level_rewards.sum(dim=-1)

    if config is not None:
        try:
            clipv = float(config.get("grpo_score_clip", 0.0))
        except Exception:
            clipv = 0.0
        if clipv > 0:
            scores = torch.clamp(scores, -clipv, clipv)

    # Optional is_gt mask: when gt_exclude_from_stats=True, GT rows are excluded
    # from group mean/std but still receive advantages (computed against policy stats).
    gt_exclude = False
    if config is not None:
        try:
            gt_exclude = bool(config.get("gt_exclude_from_stats", False))
        except Exception:
            gt_exclude = False

    id2score = defaultdict(list)
    id2score_policy = defaultdict(list)  # policy-only scores for stats
    id2mean = {}
    id2std = {}
    ...
    with torch.no_grad():
        ...
        for i in range(bsz):
            ...
            # If gt_exclude, only include non-GT scores in stats
            if gt_exclude and is_gt is not None and is_gt[i]:
                pass  # skip GT from policy stats
            else:
                id2score_policy[index[i]].append(scores[i])

        # Use policy-only scores for mean/std when gt_exclude is on
        stats_source = id2score_policy if gt_exclude else id2score
        for idx in id2score:
            policy_scores = stats_source.get(idx, [])
            if len(policy_scores) == 0:
                # All members are GT — fall back to full group
                policy_scores = id2score[idx]
            if len(policy_scores) == 1:
                id2mean[idx] = torch.tensor(0.0)
                id2std[idx] = torch.tensor(1.0)
            elif len(policy_scores) > 1:
                scores_tensor = torch.stack(policy_scores)
